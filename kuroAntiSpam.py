...
def checkTimeline(max_id="999999999999999999"):
    ...
    for item in r.json():
        ...
        if checkContent(item) == True:
            spammers.append(item["account"]["id"])
...
def blockInstance(instance):
    ...
    json_data = {
        "domain": instance,
        "severity": "suspend",
        "public_comment": "Temporary DDOS",
        "private_comment": "Automated block"
    }

    r = requests.post(url, data=json_data, headers=HEADERS)
    if r.status_code != 200:
        print("Failed to suspend instance")
        return None
    print("Suspended instance", instance)
